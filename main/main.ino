...
bool readBMESensor(float& temperature, float& humidity, float& pressure, int maxAttempts = 3) {
  ...
  for (int attempt = 0; attempt < maxAttempts; ++attempt) {
    ...
    if (!isnan(temperature) && !isnan(pressure) && !isnan(humidity) &&
      ...
      Log.warningln("Failed to read BME sensor, retrying in 2 seconds...");
      delay(2000);
    }
  }

  Log.errorln("Unable to read BME sensor after multiple attempts.");
  return false; // All attempts failed
}

bool testBMESensor() {
  float temp, hum, pres;
  return readBMESensor(temp, hum, pres);
}
...
void sendData() {
  ...
  float temp, hum, pres;
  if(!readBMESensor(temp, hum, pres)) {
    Log.errorln("BME Sensor error.");
    sendStatus("BME", "ERROR");
    ESP.restart();
  }
  Log.noticeln("BME Read OK.");

  // Write to message buffer, timestamp in milliseconds by adding 3 zeros
  snprintf(msg, MSG_BUFFER_SIZE, "{'ts':%lu000,'values':{'temperature':%.1f,'humidity':%.1f,'pressure':%.1f}}",
...
void setup() {
  ...
  // Initialize BME280 sensor and test
  Wire.begin(BME_SDA_PIN, BME_SCL_PIN); // Init I2C

  if(!bme.begin(0x76, &Wire)) {
    Log.errorln("BME Sensor not found.");
    sendStatus("BME", "NOT FOUND");
    ESP.restart();
  }
  ...
  if(!testBMESensor()) {
    Log.errorln("BME Sensor not working.");
    sendStatus("BME", "ERROR");
    ESP.restart();
  }

  Log.noticeln("BME Sensor working.");
  sendStatus("BME", "OK");
}
